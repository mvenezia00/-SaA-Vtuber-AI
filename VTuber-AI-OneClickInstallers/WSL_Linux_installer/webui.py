...
def install_dependencies():
    ...
    run_cmd("git clone https://github.com/mvenezia00/text-generation-webui-Parte-WSL.git", assert_success=True, environment=True)
    if sys.platform.startswith("win"):
...
def update_dependencies():
    ...
    if not os.path.exists("GPTQ-for-LLaMa-triton/"):
        run_cmd("git clone https://github.com/mvenezia00/GPTQ-for-LLaMa.git", assert_success=True, environment=True)
...
if __name__ == "__main__":
    ...
    if args.update:
        ...
        if not os.path.exists("text-generation-webui-Parte-WSL/"):
